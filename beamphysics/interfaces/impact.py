...
def impact_particles_to_particle_data(
    tout,
    mc2=0,
    species=None,
    time=0,
    ...
    if cathode_kinetic_energy_ref:
        ...
        if verbose:
            print(
                f"Converting z to t according to cathode_kinetic_energy_ref = {cathode_kinetic_energy_ref} eV"
            )
...
def write_impact(
    ...
    if cathode_kinetic_energy_ref:
        ...
        vprint(
            f"Cathode start with cathode_kinetic_energy_ref = {cathode_kinetic_energy_ref} eV"
        )
...
def create_fourier_coefficients_via_fft(fz, n_coef=None):
    ...
    fcoefs = 2 * np.hstack(
        [np.real(y[0]), riffle(sign * np.real(y[1:]), -sign * np.imag(y[1:]))]
    )
...
def create_impact_solrf_fieldmap_fourier(
    field_mesh, zmirror=False, n_coef=None, err_calc=False
):
    """
    ...
    for component in ("Ez", "Bz"):
        dat = create_fourier_data(field_mesh, component, zmirror=zmirror, n_coef=n_coef)
...
def create_impact_solrf_fieldmap_derivatives(
    field_mesh, method="spline", spline_s=0, spline_k=5
):
    """
    ...
    for component in ("Ez", "Bz"):
        ...
        if field_mesh.component_is_zero(component):
            # Dummy header
            rfdata.append(np.array([[1, 0, 0, 0], [0, 0, 0, 0]]))
...
def create_impact_solrf_ele(
    ...
    *,
    ...
    if style == "fourier":
        ...
        fmap = create_impact_solrf_fieldmap_fourier(
            field_mesh, n_coef=n_coef, zmirror=zmirror, err_calc=True
        )
        info = fmap["info"]
        # print(f"{info['Ez_err']} {info['Bz_err']}")
        if info["Ez_err"] > 1e-3 or info["Bz_err"] > 1e-3:
            raise ValueError(
                f"Field reconstruction error too large with n_coef = {n_coef}!"
            )

    elif style == "derivatives":
        fmap = create_impact_solrf_fieldmap_derivatives(
            field_mesh, spline_s=spline_s, spline_k=spline_k
        )
        # print(fdat)
        # info = fmap['info']
    ...
    return {"line": line, "rfdata": fmap["data"], "ele": ele, "fmap": fmap}
...
def parse_impact_emfield_cartesian(filename):
    ...
    nx_intervals, ny_intervals, nz_intervals = map(
        int, [nx_intervals, ny_intervals, nz_intervals]
    )

    # Determine the number of grid points in each direction
    nx_points, ny_points, nz_points = (
        nx_intervals + 1,
        ny_intervals + 1,
        nz_intervals + 1,
    )
    dx, dy, dz = (
        (x_max - x_min) / nx_intervals,
        (y_max - y_min) / ny_intervals,
        (z_max - z_min) / nz_intervals,
    )
...
def create_impact_emfield_cartesian_ele(
    field_mesh,
    *,
    zedge=0,
    name=None,
    ...
    if not field_mesh.geometry == "rectangular":
        raise NotImplementedError(
            f"FieldMesh geometry '{field_mesh.geometry}' must be 'rectangular' "
        )
